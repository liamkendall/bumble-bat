...
####Lanuza plotting code

all_spp_12 <- function(PC, x="PC1", y="PC2") {
  # x="PC1"
  #  y="PC2"
...
all_spp_23 <- function(PC, x="PC2", y="PC3") {
  # x="PC1"
  #  y="PC2"
  # PC being a prcomp object
  data <- data.frame(PC$S)
  ...
  rownames(PC$L) <- c("Flower width", "Flower length", "Style length", "Plant height")
  PCAloadings <- data.frame(Variables = rownames(PC$L), PC$L)
  plot <- plot + annotate("text", x = (PCAloadings$PC2*3), 
                          y = -(PCAloadings$PC3*3),#+c(0,0,0,0,0.4,0)),
                          label = PCAloadings$Variables, color="black",fontface =2,size=4)
...
all_spp_13 <- function(PC, x="PC1", y="PC3") {
  # x="PC1"
  #  y="PC2"
  # PC being a prcomp object
  data <- data.frame(PC$S)
